...
void TCPReceiver::receive( TCPSenderMessage message )
{
  ...
  if(message.FIN) fin_received_=true;
  first_index_in_TCP=message.seqno.unwrap(start_stream_,checkpoint_);
    bool is_vaild_substr=true;
  if(first_index_in_TCP==0&&!message.SYN) is_vaild_substr=false;
  if(first_index_in_TCP!=0) first_index_in_TCP--;
  ...
  if(fin_received_&&(now_index_in_reassembler==reassembler_.get_eof())) str_finished_=true;
  if(syn_received_){
    next_seq_no_ = message.seqno.wrap(now_index_in_reassembler+1, start_stream_);
    if(str_finished_) *next_seq_no_=*next_seq_no_+1;
  }
  (void)message;
}
...
TCPReceiverMessage TCPReceiver::send() const
{
  ...
  else ret.window_size=capacity;
  ret.RST=false;
  if(reassembler_.byteStream().has_error()) ret.RST=true;
  return ret;
}
