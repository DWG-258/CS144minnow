...
uint64_t TCPSender::sequence_numbers_in_flight() const
{
  ...
  if(sequence_size_aftercut_>0) ++it;
...
void TCPSender::push( const TransmitFunction& transmit )
{
  ...
  if(is_fin_sent_||window_size_-sequence_numbers_in_flight()==0||reader().bytes_buffered()==0) break;
  }
...
void TCPSender::receive( const TCPReceiverMessage& msg )
{
  ...
  else if(window_size_==0) input_.set_error();
  //remove the acknoed sequence

  if(revice_left_window>stream_seq_||revice_left_window<=old_max_seq_) return;
  ...
  if(outstanding_sequences_.empty()) retrans_timer_.stop();
}
